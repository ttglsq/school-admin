                  ...
                  return (
                    <Card key={clsPerf.classId} className="overflow-hidden">
                      ...
                      <div className="px-4 py-3 text-sm text-muted-foreground space-y-1">
                        <p>计费规则：{durationToHours(clsPerf.duration)}课时班级，每学生每次课 ¥{getPartTimePerStudent(clsPerf.duration)}，每班每次课保底 ¥{getPartTimeMinFee(clsPerf.duration)}</p>
                        <p>本月计算：max({clsPerf.studentCount}人 × ¥{getPartTimePerStudent(clsPerf.duration)}, ¥{getPartTimeMinFee(clsPerf.duration)}) × 实际出勤周数 = max(¥{(clsPerf.studentCount * getPartTimePerStudent(clsPerf.duration)).toFixed(0)}, ¥{getPartTimeMinFee(clsPerf.duration)}) × 实际出勤周数 = <span className="font-semibold text-foreground">¥{clsPerf.partTimeWage.toFixed(2)}</span></p>
                      </div>
                    </Card>
                  );
                }
                ...
                <Card key={clsPerf.classId} className="overflow-hidden">
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-muted/30">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      <span className="text-muted-foreground">管理费 <span className="font-semibold text-foreground">¥{clsPerf.managementFee.toFixed(2)}</span></span>
                  ...
                  <div className="flex items-center justify-end gap-6 px-4 py-2.5 bg-muted/20 border-t text-sm">
                    ...
                    <span>管理费: <span className="font-semibold">¥{clsPerf.managementFee.toFixed(2)}</span></span>
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-2 text-xs text-muted-foreground">
                    ...
                    <div className="space-y-1">
                      ...
                      <p>复述工资: 3课时 ¥3/人次, 4课时 ¥4/人次, 2课时无 ｜ 班级管理费: A级老师 2课时¥160/3课时¥200/4课时¥220, B/C级老师 2课时¥140/3课时¥180/4课时¥200，按实际出勤周数比例计算（5周全勤为全额，少一周减20%）</p>
                      <p>D级兼职老师：无听力/复述/管理费考核，工资与课时挂钩 —— 1课时班级保底¥60/每生¥20，2课时班级保底¥120/每生¥40，3/4课时班级保底¥180/每生¥60；≤3学生按保底，超出按实际人数×单价，按实际出勤周数计薪</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
